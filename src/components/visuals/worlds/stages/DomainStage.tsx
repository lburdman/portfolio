 ...
 *
 * The switch is exhaustive over `Domain['stage']`: adding a seventh kind to
...
export function DomainStage({ domain, active }: StageProps) {
  switch (domain.stage) {
    case 'completion':
      return <CompletionStage domain={domain} active={active} />;
    case 'landscape':
    ...
    default: {
      ...
      // a `null` so that a seventh stage kind fails the build loudly instead
      // of shipping an empty frame — the same choice `getDomain` already makes.
