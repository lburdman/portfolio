 ...
 * are not inside an island. Neither may grow a second copy of the other's
 * queries: `REDUCED_MOTION_QUERY` is declared here and re-exported from nowhere
 * else, so the two cannot drift.
 *
...
export function matchesMedia(query: string): boolean {
  return listOf(query)?.matches ?? false;
}
 ...
 *
 * Listening is what makes toggling the OS motion setting take effect without a
 * reload, which docs/MOTION_SYSTEM.md §6 requires of every gate.
 */
